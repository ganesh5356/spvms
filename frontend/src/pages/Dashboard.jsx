...
export default function Dashboard() {
  ...
  const [stats, setStats] = useState({ vendors: 0, prs: 0, pos: 0, posWithAllGst: 0 })
  ...
  useEffect(() => {
    async function loadStats() {
      try {
        ...
        const posWithAllGstCount = Array.isArray(poRes) ? poRes.filter(po => 
          po.cgstAmount && po.cgstAmount > 0 && 
          po.sgstAmount && po.sgstAmount > 0 && 
          po.igstAmount && po.igstAmount > 0
        ).length : 0
        setStats({
  ...
  return (
    <section className="view">
      <div className="dashboard-stats">
        ...
        <div className="stat-card-dashboard" onClick={() => {
          setTab('po')
          setPoFilter('all')
      ...
      <nav className="tabs">
        ...
        <button className={`tab ${tab==='po'?'active':''}`} onClick={() => {
          setTab('po')
          setPoFilter('all')
        }}>Orders</button>
